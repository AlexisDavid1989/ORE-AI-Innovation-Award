...
5. Submit before the deadline: Push your project to its own branch in the GitLab repository, and upload any demo video to the award SharePoint page at [SHAREPOINT_LINK], before October 7, 2026, 23:59 UK time.
...
- Sub-agent token notice. The totals include only model requests present in the local VS Code logs. A sub-agent invocation may be visible without its child model calls being recorded, so sub-agent token usage may not be included in the total.
...
Answer: Not a single spreadsheet required. Opening the project in VS Code auto-generates the usage report from your local Copilot logs - it costs zero tokens and reads data that's already there. Sub-agent child model calls may not be included when they are absent from those logs.
